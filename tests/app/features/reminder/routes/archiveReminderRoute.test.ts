...
describe("Testes da rota de archiveReminder.", () => {
    let token = "";
    let testReminderId = "";
    ...
    test("Deve retornar erro de acesso.", async () => {
        const result = await request(app).put(`/archivereminder/123`)
    ...
    async () => {
        ...
        const result = await request(app)
        .put(`/archivereminder/9bb01982-1b42-456e-ba0c-ff314c835d51`)
        .set("authorization", `bearer ${token}`);
        const message = result._body.message;
    ...
    test("Deve retornar uma confirmação da arquivação/desarquivação do recado.", 
    async () => {
        const result = await request(app)
        .put(`/archivereminder/${testReminderId}`)
        .set("authorization", `bearer ${token}`);
        const message = result._body.message;
