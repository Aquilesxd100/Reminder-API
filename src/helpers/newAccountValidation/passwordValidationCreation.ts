export default function passwordValidation(password : string):boolean | string {
    ...
    if (password.length < 4) {
        validation = "A senha precisa ter ao menos 4 dígitos.";
    };
    if (password.length > 10) {
        validation = "A senha pode ter no máximo 10 dígitos.";
    }
    ...
    if (auth === false) {
        validation = "A senha precisa ter ao menos um número.";
    }
    auth = false;
    ...
    if (auth === false) {
        validation = "A senha precisa ter ao menos uma letra.";
    }
    return validation;
}